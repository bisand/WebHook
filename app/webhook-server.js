...
handler.on('*', function (event) {
  ...
  switch(event.path) {
    case '/blogevents':
      sshExec('rm -rf /tmp/' + event.payload.repository.name + ' && git clone ' + event.payload.repository.clone_url + ' /tmp/' + event.payload.repository.name + ' && cd /tmp/' + event.payload.repository.name + '/ && ls -hal && dotnet publish -o ./publish/', 'root@gollum', function (err, stdout, stderr) {
